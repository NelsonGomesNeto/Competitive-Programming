...
const std::unordered_set<std::string> kGreaterProperties = {
    "cat",
    "trees",
};
const std::unordered_set<std::string> kLessProperties = {
    "pomeranians",
    "goldfish",
};
    ...
    for (const auto [i, a] : aunts | std::views::enumerate) {
      ...
      const bool is_target = std::ranges::all_of(
          a.properties, [&](const std::pair<std::string, int>& kv) {
            ...
            if (kGreaterProperties.contains(k)) {
              return v > it->second;
            } else if (kLessProperties.contains(k)) {
              return v < it->second;
            } else {
              return v == it->second;
            }
