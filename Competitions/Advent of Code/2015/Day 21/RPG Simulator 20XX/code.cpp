...
struct Game {
  ...
  void MinGoldRec(int& best, std::vector<int>& best_picks, Being player,
    ...
    MinGoldRec(best, best_picks, player, i + 1, item_budget, gold, picks);
    ...
    if (budget) {
      ...
      picks.push_back(i);
      MinGoldRec(best, best_picks, player, i + 1, item_budget, gold + item.cost,
                 picks);
    }
  }
  int MinGold() const {
    ...
    MinGoldRec(best, best_picks, initial_player);
