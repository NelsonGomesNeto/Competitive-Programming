...
struct Snapshot {
  ...
  int Solve() const {
    ...
    for (int i = 0; i < aboves_dag.size(); ++i) {
      ...
      // if (all_above_has_at_least_two_dep) continue;
      ...
      while (!q.empty()) {
        const int u = q.front();
        q.pop();
        if (fallen.count(u)) continue;
        fallen.insert(u);

        const std::vector<int>& aboves = aboves_dag[u];
        std::vector<int> to_fall;
        for (const int above : aboves) {
          int not_fallen_dep_cnt = 0;
          for (const int above_below : belows_dag[above]) {
            if (fallen.count(above_below)) continue;
            ++not_fallen_dep_cnt;
          }
          if (not_fallen_dep_cnt >= 1) continue;
          to_fall.push_back(above);
        }

        // std::cout << "\t" << GetName(u) << " " << to_fall.size() << "\n";

        if (to_fall.empty()) continue;
        for (const int v : to_fall) q.push(v);
      }
      std::cout << GetName(i) << " - falls: " << fallen.size() - 1 << "\n";
      ans += fallen.size() - 1;
    }
    return ans;
  }
