...
const lli kExtraPosRange = 1e1;
const lli kExtraVelRange = 1e1;
...
struct Hailstone {
  ...
  double MinDist(const Hailstone& other, const Coordinate lol) const {
    lli t = 0;
    ...
    return abs(this_pos.Get(lol) - other_pos.Get(lol));
...
struct Hailstones {
  ...
  std::pair<lli, lli> GetPosBounds(const Coordinate c) const {
    ...
    return std::make_pair(lo - kExtraPosRange, hi + kExtraPosRange);
    // return std::make_pair(200000000000000, 400000000000000);
  }
  std::pair<lli, lli> GetVelBounds(const Coordinate c) const {
    ...
    return std::make_pair(lo - kExtraVelRange, hi + kExtraVelRange);
  ...
  double SumMinDist(const Hailstone& bullet, const Coordinate c) const {
    double res = 0;
    // for (int i = 0; i < 10; ++i) {
    ...
    int lol = 3;
    for (const Hailstone& h : hs) {
      res += bullet.MinDist(h, c);
      if (--lol == 0) break;
    }
    // for (int i = 0; i < 3; ++i) res += bullet.MinDist(hs[i], c);
    return res;
  }
  Hailstone FindBullet() const {
    ...
    std::vector<std::vector<TernarySearchParams>> tsps_by_coordinate;

    tsps_by_coordinate.emplace_back();
    for (const auto c : kAllCoordinates) {
      const auto [pos_lo, pos_hi] = GetPosBounds(c);
      const auto [vel_lo, vel_hi] = GetVelBounds(c);
      for (const auto [lo, hi, on_pos] :
           std::vector<std::tuple<lli, lli, bool>>{{pos_lo, pos_hi, true},
                                                   {vel_lo, vel_hi, false}})
        tsps_by_coordinate.back().emplace_back(lo, hi, on_pos, c, bullet);
    }

    for (int k = 0; k < 1; ++k) {
      std::vector<int> hehe;
      for (int lol = 0; lol < tsps_by_coordinate.size(); ++lol)
        hehe.push_back(lol);
      do {
        for (int lol : hehe) {
          // for (auto& tsps : tsps_by_coordinate) {
          auto& tsps = tsps_by_coordinate[lol];
          // const Coordinate curr = tsps[0].coordinate;
          // std::map<Coordinate, lli> prv_pos, prv_vel;
          // for (const Coordinate c : kAllCoordinates) {
          //   if (c == curr) continue;
          //   prv_pos[c] = bullet.pos.Get(c);
          //   prv_vel[c] = bullet.vel.Get(c);
          //   bullet.pos.Set(c, 0);
          //   bullet.vel.Set(c, 0);
          // }
          for (int i = tsps.size() - 1; i >= 0; --i) {
            ...
            if (i + 1 == tsps.size()) {
              tsp.eval = [&, c = tsp.coordinate](const lli value) {
                // return SumMinDist(bullet, c);
                return SumMinDist(bullet, Coordinate::X) +
                       SumMinDist(bullet, Coordinate::Y) +
                       SumMinDist(bullet, Coordinate::Z);
              };
            } else {
          ...
          // for (const Coordinate c : kAllCoordinates) {
          //   if (c == curr) continue;
          //   bullet.pos.Set(c, prv_pos[c]);
          //   bullet.vel.Set(c, prv_vel[c]);
          // }
        }
        break;
      } while (std::next_permutation(hehe.begin(), hehe.end()));
      break;
    }
  ...
  lli Solve() const {
    ...
    for (const Coordinate c : kAllCoordinates)
      std::cout << "\t" << CoordinateName(c)
                << " dist: " << SumMinDist(bullet, c) << "\n";
