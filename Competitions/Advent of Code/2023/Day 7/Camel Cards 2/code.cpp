...
const std::string cards_order = "AKQT98765432J";
...
struct Hand {
  ...
  bool operator<(const Hand& other) const {
    return type < other.type ||
           (type == other.type && cards_rank < other.cards_rank);
  }
  ...
  void Build() {
    ...
    auto j = cnt.find('J');
    if (j != cnt.end() && cnt.size() > 1) {
      const int f = j->second;
      cnt.erase(j);
