...
struct Problem {
  ...
  int FindBestPath() {
    ...
    prv.clear();
    filled_map = map;
    auto MarkSteps = [&](const std::vector<Vec>& steps) {
    ...
    while (!pq.empty()) {
      auto [pos, dir, score, steps] = pq.top();
      pq.pop();
      ...
      if (pos == end_pos) {
        if (best_path_score == kInfinity) best_path_score = score;
        if (score == best_path_score) MarkSteps(steps);
        else break;
        continue;
      }
      ...
      auto AddNext = [&](const Vec nxt_pos, const Direction nxt_dir,
        ...
        pq.emplace(nxt_pos, nxt_dir, nxt_score, steps);
      };
