...
struct Machine {
  ...
  int Dfs(const int positive_mask, int parity_mask, int depth = 0) {
    if (positive_mask == 0) {
      return 0;
    }
    ...
    if (depth == 0)
      std::println("{}{}", std::string(depth, '\t'), dfs_joltages);
    ...
    for (const auto& [deltas, presses] : buttons_combinations) {
      ...
      std::println("{}{} + 2*{}", std::string(depth + 1, '\t'), presses,
                   dfs_joltages);
      ans = std::min(ans, presses + 2 * Dfs(new_positive_mask, new_parity_mask,
                                            depth + 1));
