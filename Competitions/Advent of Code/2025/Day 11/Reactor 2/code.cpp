...
struct Reactor {
  ...
  int problematic_device_1, problematic_device_2;
  std::vector<std::vector<std::vector<lli>>> memo;
  lli Dfs(const int u, bool done_1, bool done_2, const int target) {
    if (u == target) {
      return done_1 && done_2;
    }
    done_1 |= u == problematic_device_1;
    done_2 |= u == problematic_device_2;
    lli& ans = memo[u][done_1][done_2];
    if (ans != -1) return ans;

    // Remember to double-check if there aren't any cycles!
    ans = 0;
    for (const int v : graph[u]) {
      ans += Dfs(v, done_1, done_2, target);
    }
    return ans;
  }
  lli CountPaths() {
    memo.resize(all_devices.size(),
                std::vector<std::vector<lli>>(2, std::vector<lli>(2, -1)));
    problematic_device_1 = GetId("dac");
    problematic_device_2 = GetId("fft");
    return Dfs(GetId("svr"), false, false, GetId("out"));
  }
};
template <>
...
int main() {
  ...
  const lli ans = reactor.CountPaths();
  std::println("Ans: {}", ans);
