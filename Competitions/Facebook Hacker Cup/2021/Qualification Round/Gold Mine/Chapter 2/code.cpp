...
struct Node
{
  int u, jumps; lli d;
  lli blocked, used;
  bool operator<(const Node &other) const { return d < other.d || (d < other.d && jumps > other.jumps) || (d == other.d && jumps == other.jumps && (__builtin_popcount(used) < __builtin_popcount(other.used) || __builtin_popcount(blocked) < __builtin_popcount(other.blocked))); }
};
lli dijkstra()
{
  ...
  while (!pq.empty())
  {
    ...
    if (now.used == allUsed || now.blocked == allBlocked) continue;
