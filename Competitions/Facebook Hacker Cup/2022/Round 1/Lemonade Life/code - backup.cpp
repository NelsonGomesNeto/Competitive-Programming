...
lli dijkstra()
{
  ...
  while (!pq.empty())
  {
    ...
    //   int newI = binarySearch(nowAt, newIsAbove, lo, squaredDistanceSmallerThanDD);
    //   if (newI == -1) continue;

    //   for (int j = lo; j <= newI; ++j)
    //   {
    ...
    for (int k = -5; k <= 5; ++k)
    {
      ...
      int newI = otherSideClosest[now.isAbove][now.i] + k;
      if (newI >= 0 && newI < paths[newIsAbove].size() && squaredDistanceSmallerThanDD(nowAt, paths[newIsAbove][newI]))
      {
        lli newCost = now.d + getCost(nowAt, paths[newIsAbove][newI]);
        if (newCost < costs[newIsAbove][newI])
        {
