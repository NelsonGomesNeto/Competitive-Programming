...
for each person p:
  let p be the wolf
  every person that can kill p, will kill p
    (p, the wolf, will have wolfVotes votes)
  every person that can't kill p, will vote optimally
    (everyone must vote, but no one can exceed (wolfVotes - 1) votes)
  maxFlow will optimally vote for us
