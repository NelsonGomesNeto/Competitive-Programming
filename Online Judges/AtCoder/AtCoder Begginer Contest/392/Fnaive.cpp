...
// is much bigger haha. O(n*lg) is FAST ()
...
struct Data {
  int value;
  Data Merge(const Data& other) { return Data{value + other.value}; }
};
...
struct TestCase {
  ...
  std::vector<int> SolveWithSegmentTreeAndInverseProblem() {
    SegmentTree<Data> segtree(n, nil);
    for (int i = 0; i < n; ++i) segtree.data[i] = Data{1};
    segtree.Build();
    std::vector<int> ans;
    for (int i = n - 1; i >= 0; --i) {
      int lo = 0, hi = n - 1;
  ...
  bool Run() {
    ...
    std::vector<int> ans = SolveWithBuckets();
