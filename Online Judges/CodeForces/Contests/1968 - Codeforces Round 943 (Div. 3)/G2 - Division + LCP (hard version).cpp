...
/*
We must be able to iterate over the entire output.
So, let's do that and cache the responses as much as possible.
*/
...
struct TestCase {
  ...
  void Solve(const int lo, const int hi) {
    for (int k = lo; k <= hi; ++k) {
      ans[k] = BiggestSubstringWithAtLeastKOccurrences(k);
    }
  }
