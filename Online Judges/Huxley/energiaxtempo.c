...
long double bt(struct programa p[], int i, int j, long double dp[programas + 1][niveis + 1])
{
  ...
  if (dp[i][j] == -1) //Descobre qual o melhor caminho de dp[i][j]
  {
    dp[i][j] = p[i].edp[j] + bt(p, i + 1, j, dp); //Define ele seguindo reto
    int k;
    for (k = 0; k < niveis; k ++) //Testa todos os caminhos diferentes
    {
      if (k != j) //Escolhendo sempre o melhor
      {
        dp[i][j] = min(dp[i][j], bt(p, i + 1, k, dp) + shift + p[i].edp[k]);
      }
    }
  }
  return(dp[i][j]); //Se já souber qual é o melhor, já retorna ele
}
